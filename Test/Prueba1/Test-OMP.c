...
{
    #pragma omp parallel default(none) shared(pMatrix, Rows, Cols)
 {
    ...
    for (int i = 0; i < Rows; i++) {
        for (int j = 0; j < Cols; j++) {
              ...
              pMatrix[i][j] = Random%10;  // Llenar con valores aleatorios entre 0 y 9
        }
    }
 }
}
...
{
    #pragma omp parallel for default(none) shared(pMatrixA, pMatrixB, pResultMatrix, Rows, Cols)   schedule(dynamic)
    for (int i = 0; i < Rows; i++) {
        for (int j = 0; j < Cols; j++) {
