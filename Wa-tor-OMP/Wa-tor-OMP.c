...
int main(int argc, char **argv)
{
 ...
 long int NIterFishes=0;	//Number of iterated fishes.
 long int NIterSharks=0;	//Number of iterated sharks.
...
#ifdef _OPENMP
    int nThreads = omp_get_max_threads();  // Obtener número de hilos disponibles
...
printf("T.CsPar (IterateOcean) = %g sec.\n",
 ...
 if (FFMPEG)
  ...
  printf("Wa-tor ends.\n" 
        ...
        "NFinalFishes = %d,\n" 
        "NFinalSharks = %d,\n"
        "NIterFishes  = %7ld,  %2.2f%%, \n"
        "NIterSharks  = %7ld,  %2.2f%%. \n",
        SimIter,NFishes,NSharks,
        NIterFishes, (double)NIterFishes/(double)(NIterFishes+NIterSharks),
        NIterSharks, (double)NIterSharks/(double)(NIterFishes+NIterSharks)
       );
