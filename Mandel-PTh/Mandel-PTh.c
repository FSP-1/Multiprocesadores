...
// Definición de la función para convertir el índice del vector al índice de la matriz
void VectorToMatrixInd(int vindex, int *row, int *col, int NCols) {
    *row = vindex / NCols;
    *col = vindex % NCols;
}
/*---------------------------------------------------------------------------*/
void * Th_Mandel (void * pTh_Args)
{
 ...
 int MyPixel = 0; // Índice inicial de píxeles

#if (PIXEL==1)  
 while (*(pMyData->pRowToProcess) < Rows * Cols) {  // Procesar hasta el último píxel
        // Bloqueo del mutex para obtener el índice del siguiente píxel
        pthread_mutex_lock(pMyData->pth_RowToProcess_mutex);

        if (*(pMyData->pRowToProcess) < Rows * Cols) {
            MyPixel = *(pMyData->pRowToProcess);

            // Calcular el tamaño del chunk sin pasarse del número total de píxeles
            MyChunckSize = (*(pMyData->pRowToProcess) + *(pMyData->pChunckSize) <= Rows * Cols)
                              ? *(pMyData->pChunckSize)
                              : (Rows * Cols - *(pMyData->pRowToProcess));

            // Actualizar el índice del siguiente píxel
            *(pMyData->pRowToProcess) += MyChunckSize;
        }

        pthread_mutex_unlock(pMyData->pth_RowToProcess_mutex);

        #if (PRINT == 1)
        printf("Thread %d, processing pixels %d-%d.\n",
               pMyData->ThreadId, MyPixel, MyPixel + MyChunckSize - 1);
        fflush(stdout);
        #endif

        // Procesar cada píxel en el chunk
        for (int pixel = MyPixel; pixel < MyPixel + MyChunckSize; pixel++) {
            int i, j;
            // Convertir el índice del píxel al índice de la matriz
            VectorToMatrixInd(pixel, &i, &j, Cols);

            // Llamar a `PixelMandel` con los índices `[i][j]`
        ...
        #if (DEBUG == 1)
        pMyData->NRowsDone += MyChunkSize;
        #endif
    }

    if (pMyData->ThreadId != 0)  // No es el hilo principal
        ...
        if (*(pMyData->pRowToProcess) < pMyData->Rows)
           {
            ...
            //TODO: Set MyChunkSize without reach Rows 
             MyChunckSize = (*(pMyData->pRowToProcess) + *(pMyData->pChunckSize) <= pMyData->Rows)
                           ? *(pMyData->pChunckSize)
                           : (pMyData->Rows - *(pMyData->pRowToProcess));
...
int main(int argc, char **argv)
{
 ...
 //Get mem for threads. pThreads[0] not used: main instead.
 pThreads = (pthread_t *) GetMem(NThreads+1, sizeof(pthread_t), 
                                 "Main: pThreads");
 //Get mem for args to threads. Main thread computes pTh_Args[0].
 pTh_Args = (Type_Th_Mandel_Args *) GetMem(NThreads, 
...
#if(DICOTOMICA == 1)
 int min = 1;
    int max = Rows / NThreads;
    double T_min, T_max, T_med;
 ...
 printf("Iteracion: %d\n",a );
 printf(" \n");    
 // Reiniciar las matrices de colores
 ...
 for (int t=1; t<NThreads; t++) 
   ...
   gettimeofday(&tv2, &tz);
   T_min = (tv2.tv_sec - tv1.tv_sec) + (tv2.tv_usec - tv1.tv_usec) * 1e-6   ;
    
    printf("El tamaño de chunk min es: %d\n", min);
    printf("Tiempo de ejecución para chunk 'min': %lf ms\n", T_min);
...
// Max ----------------------------
 ...
 for (int t=0;t<NThreads;t++) 
     {
            pTh_Args[t].pRowToProcess	= &RowToProcess;
            pTh_Args[t].pChunckSize = &max; // Asignamos `max` para esta iteración
     }
 ...
 for (int t=1; t<NThreads; t++) 
     {
      ...
      fflush(stdout);
        ...
        gettimeofday(&tv2, &tz);
   T_max = (tv2.tv_sec - tv1.tv_sec) + (tv2.tv_usec - tv1.tv_usec) * 1e-6  ;
     printf("El tamaño de chunk max es: %d\n", max);
     printf("Tiempo de ejecución para chunk 'max': %lf ms\n", T_max);
 ...
 for (int t=1; t<NThreads; t++) 
     {
      ...
      fflush(stdout);
        ...
        gettimeofday(&tv2, &tz);
       T_med = (tv2.tv_sec - tv1.tv_sec) + (tv2.tv_usec - tv1.tv_usec) * 1e-6  ; 

      
      printf("El tamaño de chunk med es: %d\n", med);
      printf("Tiempo de ejecución para chunk 'med': %lf ms\n", T_med);
        

 // Comparación y ajuste en el ciclo
       if (T_min < T_max) {
            max = med;
            T_max = T_med;
        } else {
     ...
     int optimalChunk = (T_min < T_max) ? min : max;
    printf("El tamaño de chunk óptimo es: %d\n", optimalChunk);
     printf(" \n");
 }
...
Th_Mandel((void *)&pTh_Args[0]);
 ...
 #if (DEGUG==1)
     printf("Main     did %d rows.\n", pTh_Args[0].NRowsDone);
 for (int t=1;t<NThreads;t++)
